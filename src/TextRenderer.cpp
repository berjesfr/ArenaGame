    ...
    : p_TextShader(shader)
{
    ...
    p_TextShader.SetMatrix4("projection", projection);
...
void TextRenderer::load(std::string font, unsigned int fontSize)
{
    ...
    for (GLubyte c = 0; c < 128; c++) // lol see what I did there 
    {
        ...
        p_Characters.insert(std::pair<char, Character>(c, character));
    }
...
void TextRenderer::render_text(std::string text, float x, float y, float scale, glm::vec3 color)
{
    ...
    p_TextShader.Use();
    p_TextShader.SetVec3F("textColor", color.x, color.y, color.z);
    ...
    for (c = text.begin(); c != text.end(); c++) {
        ...
        float ypos = y + (p_Characters['H'].Bearing.y - ch.Bearing.y) * scale;
